...
if(!array_key_exists('ID',$_SESSION) && empty($_SESSION['ID'])){
   $query1 = "SELECT score FROM algebra_gimnaziu WHERE ID =:id_pr";
   $query = "SELECT answear FROM algebra_gimnaziu  WHERE ID =:id_pr";
   ...
   if($row['answear'] == $post and $user->insertPair()){
   ...
   else if($user->alreadyResolved() == true){
      $update_score = array(
